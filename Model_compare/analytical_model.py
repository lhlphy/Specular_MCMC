...
def A_Fresnel(Theta = 0, A_normal = 0, I_angle = -1, inc = 90, offset = 0):
...
def F_thermal(Theta_array, AB, F=0, Tss = Tss_ref, Rp2Rs = 0, offset = 1.5):
    ...
    lam_array = np.linspace(lam1, lam2, 50)
    ...
    for i, Theta in enumerate(Theta_array):
        ...
        if Theta < alpha or Theta > 2*np.pi - alpha: # transit
            results.append(-np.pi *  int_result)
            # print((Rp/Rs)**2)
            continue
        ...
        lam_list = np.linspace(lam1, lam2, 50)
...
## OLD SIMPLE MODEL
# def F_specular(Theta_array, AB, Rp2Rs, offset=1.5):
...
def Fp2Fs(Theta_array, AB, alpha_ellip, delta =0, Tss = Tss_ref, Rp2Rs = 0, offset = 1.5, F=0):
    offset = 0.4 # offset = 0.4
    return F_thermal(Theta_array, AB, F, Tss, Rp2Rs, offset) + F_specular(Theta_array, AB, Rp2Rs) + F_ellip(Theta_array, alpha_ellip) + delta
