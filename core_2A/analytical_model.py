...
try:
    from core_2A.fresnel_lookup import lookup_fresnel
except ImportError:
    from fresnel_lookup import lookup_fresnel
...
def A_Fresnel(Theta = 0, A_normal = 0, I_angle = -1, inc = 90):
    ...
    return lookup_fresnel(A_normal, I_angle)
...
#         # 定义采样�?#         phi_list = np.linspace(-np.pi / 2, np.pi / 2, 180)
#         theta_list = np.linspace(np.pi/2 - Theta, 3*np.pi/2 - Theta, 180)
# #         # 构造广播数�?#         theta_array = theta_list[:, np.newaxis]  # shape (180, 1)
#         phi_array = phi_list[np.newaxis, :]      # shape (1, 180)
# 
#         # # 矢量化计�?I_matrix
...
def F_thermal(Theta_array, A_th, F=0, Tss=Tss_ref, Rp2Rs=PPs.Rp2Rs, inc=90, lam1=lam1, lam2=lam2):
    ...
    for Theta in theta_view:
        ...
        thermal_band = band_integral(Toy_model(cos_zenith, A_th, F, Tss), lam1, lam2, n_lam=8)
        lambert_loss = 1 - A_th
        integrand = -thermal_band * cos_phi**2 * np.cos(Theta + theta_array) * lambert_loss
...
def Fp2Fs(Theta_array, AB=0, A_th=0, F=0, alpha_ellip=0, co1=Co1, co2=Co2, Tss = Tss_ref, Rp2Rs = PPs.Rp2Rs, inc = 90, alpha=PPs.alpha, params = [], AA =-1):
    if len(params) != 0:
    ...
    if AA != -1:
        print("AA fixed!")
        AB = AA
    return (F_thermal(Theta_array, A_th, F, Tss, Rp2Rs, inc) + F_specular(Theta_array, AB, Rp2Rs, inc, alpha)) *Eclipse(Theta_array, Rp2Rs, inc, alpha) + delta + F_Transit(Theta_array, Rp2Rs, co1, co2, inc, alpha)
