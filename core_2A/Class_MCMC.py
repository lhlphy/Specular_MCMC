...
import emcee
import corner
from core_2A.analytical_model import Fp2Fs
import arviz as az
...
class MCMC:
    def __init__(self, target_name, file_name, sigma=10, ndim=10, nwalkers=120, nsteps=2000, burnin=1000):
        ...
        self.nsteps = nsteps
        self.burnin = burnin
        self.labels = [r'$A_{\lambda}$', r"$A_{\rm th}$", r"$T_{\rm sub}$", "Rp/Rs", "F", "inc", r"$\alpha$", "u1", "u2", "delta"]
    ...
    def _initial_positions(self):
        ...
        initial[:, 1] = np.random.uniform(low=0.0, high=0.7, size=self.nwalkers)
        ...
        initial[:, 7] = truncnorm.rvs(a, np.inf, loc=self.Co1, scale=self.coeff_sigma, size=self.nwalkers)

        a = (0 - self.Co2) / self.coeff_sigma
        initial[:, 8] = truncnorm.rvs(a, np.inf, loc=self.Co2, scale=self.coeff_sigma, size=self.nwalkers)

        initial[:, 9] = np.random.uniform(low=-10, high=10, size=self.nwalkers)
        return initial
    ...
    def log_prior(self, params):
        AB, A_th, Tss, Rp2Rs, F, inc, alpha, co1, co2, delta = params
    ...
    def sample(self, resume=False, nsteps=None):
        run_steps = self.nsteps if nsteps is None else nsteps
        prior_chain = None
        ...
        if resume:
            ...
            if initial.shape != (self.nwalkers, self.ndim):
                raise ValueError("Saved chain shape does not match current nwalkers/ndim settings.")
        else:
        ...
        new_log_likelihood = sampler.get_blobs()
        self.chain = new_chain if prior_chain is None else np.concatenate([prior_chain, new_chain], axis=0)
        self.log_posterior_chain = (
            new_log_posterior
            if prior_log_posterior is None
            else np.concatenate([prior_log_posterior, new_log_posterior], axis=0)
        )
        self.log_likelihood_chain = (
            new_log_likelihood
            if prior_log_likelihood is None
            else np.concatenate([prior_log_likelihood, new_log_likelihood], axis=0)
        )
        samples = self._save_chain_products(self.chain, self.log_posterior_chain, self.log_likelihood_chain)
        self.sampler = sampler
        r_hat_values = self._compute_rhat_from_chain(self.chain)
        self.r_hat = r_hat_values
