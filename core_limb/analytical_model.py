...
try:
    from core_limb.fresnel_lookup import lookup_fresnel
except ImportError:
...
def F_thermal(Theta_array, AB, F=0, Tss=Tss_ref, Rp2Rs=PPs.Rp2Rs, inc=90, lam1=lam1, lam2=lam2):
    ...
    for Theta in theta_view:
        ...
        zenith_obs = np.arccos(-np.cos(theta_array + Theta) * cos_phi)
        thermal_band = band_integral(Toy_model(cos_zenith, AB, F, Tss), lam1, lam2, n_lam=8)
        fresnel_loss = 1 - A_Fresnel(A_normal=AB, I_angle=zenith_obs)
        integrand = -thermal_band * cos_phi**2 * np.cos(Theta + theta_array) * fresnel_loss
...
def Fp2Fs(Theta_array, AB=0, F=0, alpha_ellip=0, co1=Co1, co2=Co2, Tss = Tss_ref, Rp2Rs = PPs.Rp2Rs, inc = 90, alpha=PPs.alpha, x_offset=0, params = [], AA =-1):
    if len(params) != 0:
    ...
    return (F_thermal(shifted_theta, AB, F, Tss, Rp2Rs, inc) + F_specular(shifted_theta, AB, Rp2Rs, inc, alpha)) *Eclipse(shifted_theta, Rp2Rs, inc, alpha) + delta + F_Transit(shifted_theta, Rp2Rs, co1, co2, inc, alpha)
