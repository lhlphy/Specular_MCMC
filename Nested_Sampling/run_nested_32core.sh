...
if [ ! -d "${PROJECT_DIR}/nested_sampler" ]; then
    ...
    echo "Submit this script from the Nested_Sampling directory, or keep it next to nested_sampler/." >&2
    exit 2
fi
...
python -m nested_sampler.run \
    ...
    --sample rwalk \
    --bound multi \
    --outdir "results/server_${SLURM_JOB_ID:-manual}"
