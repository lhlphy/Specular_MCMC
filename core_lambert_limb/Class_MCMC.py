...
import emcee
import corner
from core_lambert_limb.analytical_model_Lambert import Fp2Fs
import arviz as az
...
class MCMC:
    def __init__(self, target_name, file_name, sigma=10, ndim=9, nwalkers=120, nsteps=2000, burnin=1000):
        ...
        self.nsteps = nsteps
        self.burnin = burnin
        self.labels = [r'$A_{\lambda}$', r"$T_{\rm sub}$", "Rp/Rs", "F", "inc", r"$\alpha$", "u1", "u2", "delta"]
    ...
    def _initial_positions(self):
        ...
        initial[:, 6] = truncnorm.rvs(a, np.inf, loc=self.Co1, scale=self.coeff_sigma, size=self.nwalkers)

        a = (0 - self.Co2) / self.coeff_sigma
        initial[:, 7] = truncnorm.rvs(a, np.inf, loc=self.Co2, scale=self.coeff_sigma, size=self.nwalkers)

        initial[:, 8] = np.random.uniform(low=-10, high=10, size=self.nwalkers)
        return initial
    ...
    def log_prior(self, params):
        AB, Tss, Rp2Rs, F, inc, alpha, co1, co2, delta = params
