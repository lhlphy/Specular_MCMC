...
for i, AB_S in enumerate(AB_S_list):
    ...
    dataX_2 = np.linspace(0, 2* np.pi, 500)
...
Co1, Co2 = mcmc.Co1, mcmc.Co2
dataX_2 = np.linspace(0, 2* np.pi, 500)
data_model = core.analytical_model.Fp2Fs(dataX_2, co1=Co1, co2=Co2, params=params)
...
plt.savefig('./output/ABS_compare.png')
ax.set_ylim(-20, (np.max(dataY)+mcmc.sigma) *1.1)  # 设置下限为-10，上限自动调整
plt.savefig('./output/ABS_compare_nT.png')
plt.show()
plt.close()
